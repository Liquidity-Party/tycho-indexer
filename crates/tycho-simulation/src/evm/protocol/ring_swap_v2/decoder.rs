...
impl TryFromWithBlock<ComponentWithState, BlockHeader> for RingSwapV2State {
    ...
    async fn try_from_with_header(
        ...
        let backing0 = backing_balance(&snapshot, &component_tokens[0])?;
        let backing1 = backing_balance(&snapshot, &component_tokens[1])?;
    ...
    snapshot
        ...
        .ok_or_else(|| {
            InvalidSnapshotError::ValueError(format!(
                "RingSwapV2 component {} is missing backing balance for token {token:?}",
                snapshot.component.id
            ))
        })
}
...
mod tests {
    ...
    #[tokio::test]
    async fn decodes_component_ordered_backing() {
    ...
    async fn decodes_inverted_pair_metadata_in_component_order() {
        ...
        inverted_snapshot.state.balances =
            HashMap::from([(address(1), Bytes::from(vec![7])), (address(2), Bytes::from(vec![8]))]);
